...
class Scopes(Enum):
    '''
    Enumerate the available query scopes.
    '''
    System = auto()
    Device = auto()
...
class DeviceClasses(Enum):
    '''
    Enumerate the devices classes supported by the Solar API V0 and V1.
    '''
    Inverter = auto()
    Storage = auto()
    Ohmpilot = auto()
    ...
    System = auto() # Include all device classes (used only in requests)
...
class RequestStatusCodes():
    '''
    Mapping of numerical request status codes to status text, descriptions and error flag.

    Reference: 42,0410,2012,EN  013-02062020 2.4.2 Common Response Header
    '''

    @staticmethod
    def status_text(code):
        '''
        Return a short human-readable representation of the status code
        '''
        d = {
            '0':    'OKAY',
            '1':    'NotImplemented',
        ...
        return d.get(str(code), 'UNKNOWN')

    @staticmethod
    def status_description(code):
        '''
        Return a human-readable description of the exact meaning of the status code.
        '''
        d = {
            '0':    'Request successfully finished, Data are valid',
            '1':    'The request or a part of the request is not implemented yet',
            '2':    'Instance of APIRequest created, but not yet configured',
            '3':    'Request is configured and ready to be sent',
            '4':    'Request is currently being processed (waiting for response)',
            '5':    'Response was not received within desired time',
            '6':    'Invalid arguments/combination of arguments or missing arguments',
            '7':    'Something went wrong during sending/receiving of LN-message',
            '8':    'LN-request timed out',
            '9':    'Something went wrong during parsing of successfully received LN-message',
            '10':   'Something went wrong while reading settings from local config',
            '11':   'The operation/feature or whatever is not supported',
            '12':   'The device is not available',
            '255':  'undefined runtime error'           
        }
        return d.get(str(code), 'UNKNOWN')
    ...
    def status_ok(code):
        '''
        Check whether a code represents an error condition.
        '''
        if int(code) > 0:
            return False
        else:
...
class DeviceTypes():
    '''
    Mapping of numerical device types (DT) to model names. Please note, that this is
    prepared to handle not just inverter but also other device types even though
    just inverters are mentioned in the reference doc.

    Reference: 42,0410,2012,EN  013-02062020 5.2 Inverter Device Type List    
    '''
    ...
    def model_name(dt):
        ...
        return d.get(str(dt), 'UNKNOWN')


class InverterStatusCodes():
    '''
    Mapping of numerical status codes to status tokens

    Reference: 42,0410,2012,EN  013-02062020 3.6.5 Meaning of numerical status codes
    '''
